...
class TripletLoss(nn.Module):
    ...
    def forward(self, satimg, grdimg):
        # compute image-sentence score matrix
        scores = self.sim(satimg, grdimg)

        diagonal = scores.diag().view(satimg.size(0), 1)
        d1 = diagonal.expand_as(scores)
        d2 = diagonal.t().expand_as(scores)

        mask_s = (scores >= (d1 - self.margin)).float().detach()
        cost_s = scores * mask_s + (1. - mask_s) * scores
        mask_im = (scores >= (d2 - self.margin)).float().detach()
        cost_im = scores * mask_im + (1. - mask_im) * scores
        condition = torch.eye(satimg.size(0), satimg.size(0)) * self.margin
        loss = (-cost_s.diag() + cost_s + self.margin).where(-cost_s.diag() + cost_s + self.margin > condition, condition).sum().mean() \
               + (-cost_im.diag() + cost_im + self.margin).where(-cost_im.diag() + cost_im + self.margin > condition, condition).sum().mean()
        return loss
